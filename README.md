...
| `profile.js` | **Personalización por perfil** (dinámica, client-side). Chip «Perfil» (a quién vendes: cliente final B2C / empresas B2B / partners-canal · ticket · sector) guardado en `brand_prefs`. Adapta el contenido: sustituye tokens `{{icp}}` `{{audience}}` `{{ticket}}` `{{sector}}` y muestra/oculta bloques con `data-when="audience:partners"`. Mismo método, ejemplos/mensajes según el usuario. Base para generalizar más allá de Odoo (canales indirectos, sectores, ICPs diversos). |
